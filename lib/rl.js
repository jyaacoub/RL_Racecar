...
var sampleWeighted = function(p) {
  ...
  assert(false, 'Error at sampleWeighted: '+ str(r)+','+str(c)+'||'+str(p));
}
...
TDAgent.prototype = {
  ...
  act: function(s){ 
    ...
    if(Math.random() < this.epsilon) {
      ...
      console.log(s);
      // TODO: PROPERLY INIT THE PROBABILITY DISTRIBUTION! IT IS WRONG (MISSING SOME VALUES)!
...
DQNAgent.prototype = {
  ...
  learn: function(r1) {
    ...
    // only update if previous reward is available (and therefore previous state-actions) and
    // if learning rate is greater than zero. 
    if(!(this.r0 == null) && this.alpha > 0) {
      ...
      if(this.t % this.experience_add_every === 0) {
        ...
        this.expi += 1;
        if(this.expi > this.experience_size) { this.expi = 0; } // roll over when we run out -> acts as a sort of fixed length queue
      }
      this.t += 1;
      ...
      for(var k=0;k<this.learning_steps_per_iteration;k++) {
        ...
        var e = this.exp[ri];
        this.learnFromTuple(e[0], e[1], e[2], e[3], e[4]) // tuple contains current S-A and next S-A
      }
    }
  ...
  learnFromTuple: function(s0, a0, r0, s1, a1) {
    ...
    var tmat = this.forwardQ(this.net, s1, false); // predicting reward for each action
    var qmax = r0 + this.gamma * tmat.w[R.maxi(tmat.w)]; // using max predicted reward
